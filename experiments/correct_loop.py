...
def get_score(net, dataset='test', maxnum=float('inf'), saved=False):
    ...
    for idx, file in enumerate(files):
        ...
        for obj in objects:
            if obj['label'] in selected_classes:
                ...
                num_clicks = np.mean(np.array(num_clicks))  # 一次只有一个sample
                ...
                clicks_pre_class[obj['label']].append(num_clicks)
    ...
    true_means = 0.
    for cls in iouss:
        mean_class[cls] = np.mean(np.array(iouss[cls]))
        true_means += mean_class[cls]
    true_means = true_means / 8.

    mean_clicks_class = {}
    true_clicks = 0.
    for cls in clicks_pre_class:
        mean_clicks_class[cls] = np.mean(np.array(clicks_pre_class[cls]))
        true_clicks += mean_clicks_class[cls]
    true_clicks = true_clicks / 8.

    results = {}
    results['pre_iou_class'] = iou_score
    results['true_iou_class'] = mean_class
    results['clicks_mean_over_all'] = np.mean(np.array(NUM_clicks))
    results['clicks_mean_pre_class'] = mean_clicks_class
    results['clicks_mean_over_class'] = true_clicks
    results['less2'] = less2
    # return
    return results
...
if __name__ == '__main__':
    ...
    parser.add_argument('-l', '--loop_T', type=int, default=1)  # 1, 2, 3, 4, 5 是要纠正的距离
    ...
    results = get_score(net, saved=True)

    print('Correct In the Loop with threshold ', loop_t, ':')
