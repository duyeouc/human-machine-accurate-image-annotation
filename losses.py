...
def self_critical_loss(log_probs, lengths, sampling_reward, greedy_reward):
    ...
    for i in torch.arange(reward.size(0), dtype=torch.long, device=reward.device):
        l = -1 * log_probs[i, :lengths[i]] * reward[i]
...
def self_critical_loss_with_entropy(log_probs, lengths, sampling_reward, greedy_reward, probs, beta):
    """
    Self critical sequence training loss for RL
    ...
    sampling_reward: [batch_size, ]
    greedy_reward: [batch_size, ]
    probs:  [batch_size * seq_len, 28*28+1或15*15] 概率值
    beta: 系数，初步可以设定为1e-5试一试
    """
    reward = sampling_reward - greedy_reward
    ...
    bs = reward.size(0)
    len_s = 71
    probs = probs.view(bs, 71, -1)
    # 逐个样例计算
    for i in torch.arange(reward.size(0), dtype=torch.long, device=reward.device):
        l = -1 * log_probs[i, :lengths[i]] * reward[i]  # (26)
        entropy = -1 * probs[i, :lengths[i]] * torch.log(probs[i, :lengths[i]])
        L = l + entropy
        # Expectation per polygon
        L = torch.mean(L)
        loss += L
