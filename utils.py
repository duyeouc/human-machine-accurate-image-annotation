...
def iou(vertices1, vertices2, h ,w):
    ...
    mask2 = np.array(img2)
    intersection = np.logical_and(mask1, mask2)  # 都是1
    union = np.logical_or(mask1, mask2)  # 有个1
...
def poly01_to_poly0g(poly, grid_size):
    ...
    poly = cv2.approxPolyDP(poly, 0, False)[:, 0, :]   # dp 道格拉斯~算法，多边形点曲线拟合， 并会减少点的数量
...
def prepare_delta_target(rnn_pred, polygon_gt):
    ...
    # To(0, 224)
    gt = polygon_gt.float()  # (bs, 71, 2)
    # To(0, 224)
    pred_poly_x = (pred % 28) * 8.0 + 4  # (bs, 71)
    pred_poly_y = (pred // 28) * 8.0 + 4
    # To(0, 112)
    gt = torch.floor(((gt + 0.5) / 224) * 112).int()
    pred_poly_x = torch.floor(((pred_poly_x+0.5)/224) * 112).int()
    pred_poly_y = torch.floor(((pred_poly_y+0.5)/224) * 112).int()
...
def CRS_curve(polys):
    """
    Get centripetal Catmull-Rom spline.
    :param polys: []
    :return: 曲线段x和y关于t的参数方程
    """
    bs = polys.shape[0]
...
if __name__ == '__main__':
    ...
    rnn_pred = torch.randint(low=0, high=700, size=(2, 11))
    gt = torch.randint(low=0, high=223, size=(2, 10, 2))

    x1, y1, xy = prepare_delta_target(rnn_pred, gt)
    print(rnn_pred)
    print()
    print(gt)
    print()
