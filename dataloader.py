...
# 注意这样做，trian_rl 不能用数据增强，因为里面的left WH是错的啊
class CityScape(Dataset):
    # seq_len: default 70
    def __init__(self, num, path, seq_len, transform=None, enhance=False):
        """
    ...
    # num pf samples
    def __len__(self):
        # if self.mode == 'train' and self.enhance == True:
    ...
    def __getitem__(self, index):
        ...
        def getdata(mode, flip, index, random_start=False):
            ...
            # 数值在0到1之间的polygon，就最多选取70个
            # TODO: 这里有问题=>改一下
            """
            Traceback (most recent call last):
  File "train_lstm_delta.py", line 190, in <module>
    train(config, load_resnet50=True)
  File "train_lstm_delta.py", line 57, in train
    for index, batch in enumerate(train_dataloader):
  File "/home/duye/model/pytorch-polygon-rnn/venv/local/lib/python2.7/site-packages/torch/Utils/data/dataloader.py", line 346, in __next__
    data = self._dataset_fetcher.fetch(index)  # may raise StopIteration
  File "/home/duye/model/pytorch-polygon-rnn/venv/local/lib/python2.7/site-packages/torch/Utils/data/_utils/fetch.py", line 44, in fetch
    data = [self.dataset[idx] for idx in possibly_batched_index]
  File "/home/duye/model/FPN_rnn/dataloader.py", line 199, in __getitem__
    return getdata('train', 0, index, random_start=False)  # 先不随机开始
  File "/home/duye/model/FPN_rnn/dataloader.py", line 104, in getdata
    polygon01[:point_num] = tmp
ValueError: could not broadcast input array from shape (84,2) into shape (32,2)
            """
...
def loadData(path, data_num, len_s, batch_size, shuffle=True, enhance=False):
    """
...
if __name__ == '__main__':
    ...
    #     if i>2:
    #         break
    #     print('label_onehot[0]:',v[1].shape)
    #     print('label_onehot[-2]:',v[2].shape)
    #     print('label_onehot[:-1]:', v[3].shape)
    #     print('label_index:', v[4].shape)
