...
def train(config,
    ...
    for it in range(cur_epochs, epochs):
        ...
        for index, batch in enumerate(train_dataloader):
            ...
            # get entropy
            logits1 = outdict_sample['logits']  # (bs, seq_len, 71)
            logits1 = logits1.contiguous().view(-1, 28 * 28 + 1)  # (bs*seq_len, 28*28+1)
            logits1 = torch.nn.functional.softmax(logits1, dim=1)
            logprobs = outdict_sample['log_probs']
            # 强化学习损失，logprob是两个logprob加和
            loss = losses.self_critical_loss_with_entropy(logprobs, outdict_sample['lengths'],
            ...
            logits2 = outdict_sample['delta_logits']  # (bs, seq_len, 71)
            logits2 = logits2.contiguous().view(-1, 15*15)  # (bs*seq_len, 28*28+1)
            ...
            if (index+1) % config['val_every'] == 0:
                ...
                torch.save(model.state_dict(),
                            '/data/duye/pretrained_models/FPNRLtrain/ResNext_Plus_RL_Entropy_retain_Epoch{}-Step{}_ValIoU{}.pth'.format(
                            str(it + 1),
                            str(index + 1),
                            str(val_iou_data)))
                # set to init
