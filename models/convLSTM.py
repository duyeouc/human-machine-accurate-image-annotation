...
class AttConvLSTM(nn.Module):
    def __init__(self, feats_channels=128,
                 ...
                 time_steps=71,
                 delta_grid_size=15,
                 use_bn=True, predict_delta=False, loop_T=1):
        """

        :param feats_channels: 128
        :param feats_dim: 28
        :param n_layers: 2
        :param hidden_dim: [65,16], 即隐藏层的channel数
        :param kernel_size:  3
        :param time_steps: 71=70+EOS
        :param use_bn: dependent mean/var across time-steps
        """
        super(AttConvLSTM, self).__init__()
        self.predict_delta = False  # 永远置位False
        ...
        for l in range(n_layers):
            ...
            if l != 0:
                ...
                in_channels = self.feat_channels + 2  # +prev2,prev1 不加firstV
                # 试着改一下，不用firstV了如何呢=>firstV是否是必要的?

            self.conv_x.append(
                nn.Conv2d(
                    in_channels=in_channels,
                    out_channels=4 * hidden_dim,  # 4*hidden_dim是为了一次性卷积计算三个门+候选值
                    kernel_size=self.kernel_size,
                    padding=self.kernel_size//2,  # 分辨率不变的padding设置
                    bias=not self.use_bn  # 如果用bn就不加bias
                )
            )
    ...
    def rnn_step(self, t, feats, cur_state):
        ...
        :param t: 时间步t
        :param feats: 特征map (N,128,28,28), 可以时原始Encoder后的feats_map(+3channel), 也可以是注意力机制计算后的
                        或者是第一层的输出h
        :param cur_state: 上一时间步各层的隐藏层 (n_layers, [h,c]) h:(N, hidden_dim(64-16), 28,28)
        :return:
        """
        out_state = []
        ...
        for l in range(self.n_layers):
            ...
            feats_h = conv_h(h_cur)  # 上一时间步的隐藏层进行卷积
            if self.use_bn:
                feats_h = self.bn_h[l][t](feats_h)
            # i:更新 f:遗忘 o:输出 u：c候选值
    ...
    def attn(self, feats, pre_state):
        ...
        :param feats: Encoder后的原始feats map
        :param pre_state: 上一时间步的hidden state (n_layers, [h,c])
        :return: 该时间步提取的特征,attn权重
        """
        ...
        # f_att: (N*28*28,1)
        f_att = self.fc_att(f12)  # paper中的f_att
        # f_att: (N, 28*28)
    ...
    def forward(self, feats,
        ...
        :param feats: Encoder特征map: (N,C,H,W) (N,128,28,28)
        :param temperature:< 0.01 -> Greedy, else -> multinomial with temperature
        :param mode: train_ce/train_rl/test
        :param fp_beam_size:
        :param beam_size:
    ...
    def vanilla_forward(self,
        ...
        :param use_correction:
        :return: out_dict
                test_time: pred_polys (bs, seq_len) pre-class
                           rnn_state
                           feats
                           logprob_sums: log概率之和 (decoder out + log_softmax)
                train_ce/rl:
                          pred_polys： 每步的预测点class (bs, seq_len)
                          log_probs: (bs, seq_len) 每步预测点的log_prob [ce-greedy; rl:temperature=0.3]
                          logits: (bs, seq_len, 28*28+1) 每步decoder的输出
                          lengths: (bs, ) 实际预测长度
                          rnn_state
                          feats
                          log_probsums
        """
        # TODO: 这个要改了,加上预测delta,即每一步rnn_step要改了
        # TODO：如果加快速度也可以等rnn结束之后，记录所有rnn_hidden_state,把delta_attn改成矩阵(len_s)形式, 但要注意mask
        # TODO: 先用71，之后enhance poly试一下是否管用
        ...
        if mode == 'train_ce':
            ...
            for t in range(self.time_steps):
                ...
                # 记录
                # 返回
                out_rnn_h.append(torch.cat([cur_state[-1][0], cur_state[0][0]], dim=1))  # (64+16)
            ...
            out_rnn_h = torch.stack(out_rnn_h)
            out_dict['last_rnn_h'] = out_rnn_h.permute(1, 0, 2, 3, 4)  # [bs, 71, 16, 28, 28]
            ...
            for t in range(self.time_steps):
                ...
                logprobs = F.log_softmax(cur_vertex_logits, dim=-1)  # (bs*seq_len, 28*28+1)

                if temperature < 0.01:
                    cur_logprob, cur_pred = torch.max(logprobs, dim=-1)  # greedy
                else:
                    probs = torch.exp(logprobs / temperature)
                    cur_pred = torch.multinomial(probs, 1)   # 多项分布采样:https://blog.csdn.net/qq_32806793/article/details/103159504?depth_1-utm_source=distribute.pc_relevant.none-task&utm_source=distribute.pc_relevant.none-task
                    # Get logprob of the sampled vertex
                    cur_logprob = logprobs.gather(1, cur_pred)

                    # Remove the last dimension if not 1
                    cur_pred = torch.squeeze(cur_pred, dim=-1)  # 预测的点class (bs, )
                    cur_logprob = torch.squeeze(cur_logprob, dim=-1)  # 概率值 (bs,)

                # 结束，记录lengths for each batch
                for b in range(N):
                    if out_lengths[b] != self.time_steps:
                        continue
                        # prediction has ended
        ...
        elif mode == 'interaction_loop':  # 在循环中纠正
            # pre_v2, pre_v1 = None
            v_prev2 = torch.zeros((N, 1, grid_size, grid_size), device=device)
            v_prev1 = torch.zeros((N, 1, grid_size, grid_size), device=device)
            # init hidden state
            ...
            for t in range(self.time_steps):
                ...
                # 更正curpred
                # cur_pred是预测的点index (bs, )
                cur_pred, click_num = self.correct_next_input(cur_pred, gt_28[:, t], click_num)
            ...
            if self.predict_delta:  # 这个用不到了
                ...
                delta_pred = torch.stack(out_delta_pred)
                out_dict['delta_pred'] = delta_pred.permute(1, 0).float()
            out_dict['click_num'] = click_num
        return out_dict

    # TODO: + last_rnn_h over time steps
    def beam_forward(self,
        ...
        # Setup tensors to be reused for t=0
        ...
        for t in range(0, self.time_steps):
            ...
            if t == 0:
                ...
                if fp_beam_size > 1:
                    ...
                    # feats: (fp_beam_size * bs, 128, 28, 28)
                ...
                # Update alive
                # .ne 非 (测试idx每个是否是28*28，否：True 是：False)
        ...
        """
            out_dict['pred_polys'] = tokens_table.view(full_beam_batch_size, -1)
            pred_polys: [bs*beam_size, 71],
            对于bs*beam_size, 前beam_size是属于一个样例的输出
        """
        return out_dict

    def correct_next_input(self, pred, gt, click_num):

        x = pred % self.grid_size  # (bs, )
        ...
        # 棋盘距离
        dist = torch.max(torch.abs(y_gt - y), torch.abs(x_gt - x))

        # 逐元素比较input和other
        need_correct = torch.gt(dist, self.loop_T)

        is_eos = torch.eq(gt, self.grid_size ** 2)

        clicks = need_correct & (not is_eos)

        need_correct = need_correct | is_eos

        num = sum(clicks).item()
        # 即把need_correct=True的, pred变为gt
        gt = gt.type_as(pred)
        out = torch.where(need_correct, gt, pred)

        return out, click_num + num
...
bs = 2
seq_len = 71
pred_polys torch.Size([2, 71])
rnn_state 2
logprob_sums torch.Size([2])
feats torch.Size([2, 128, 28, 28])
log_probs torch.Size([2, 71])
logits torch.Size([2, 71, 785])
